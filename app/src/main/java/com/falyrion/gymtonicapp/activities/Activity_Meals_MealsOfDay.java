...
public class Activity_Meals_MealsOfDay extends AppCompatActivity implements Adapter_MealPresets.mealPresetItemInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Using correct method name from DatabaseHelper
        Cursor cursor = databaseHelper.getConsumedMeals(date);

        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                // Table CM: 0:date, 1:meal_index, 2:amount
                // Table PM: 0:index, 1:name, 2:category, 3:calories...
                mealsList.add(new Item_MealPreset(
                        cursor.getString(cursor.getColumnIndexOrThrow("name")),  // Title
                        cursor.getString(cursor.getColumnIndexOrThrow("meal_index")),  // UUID
                        (int) cursor.getDouble(cursor.getColumnIndexOrThrow("calories")),  // Calories
                        cursor.getDouble(cursor.getColumnIndexOrThrow("amount"))  // Amount
                ));

                mealsStart.put(cursor.getString(cursor.getColumnIndexOrThrow("meal_index")), cursor.getDouble(cursor.getColumnIndexOrThrow("amount")));
            }
        }
                    ...
                    for (Item_MealPreset currentMeal : mealsList) {
                        ...
                        if (startAmount != null) {
                            if ((currentAmount > 0) && (currentAmount != startAmount)) {
                                // Delete old entry and add with new amount
                        ...
                        mealsStart.put(currentUUID, currentAmount);
                    }
